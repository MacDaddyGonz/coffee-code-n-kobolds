...
export const presetSheetValidator = v.object({
   ...
   *
   * ⚠️ **The consequence, for whoever retires Half-Orc:** this field is what makes that a
   * two-step. Removing `'half-orc'` from `speciesKeyValidator` while `race` is still
   * required and still narrow fails `npx convex deploy` against any deployment holding a
   * Half-Orc — the push validates existing documents. The migration writes `species` first,
   * and `race` is dropped after, not before.
   */

// The bestiary: 283 creatures across five content files, plus the lookups the resolver
// needs. Content lives in the files beside this one; the shape is in ./types.ts and the
// resolver is in ../resolve.ts.
//
// ⚠️ **Two hundred and fifty-three of those are transcribed from the D&D 5e (2024) SRD 5.2.1
// and thirty are authored**, and a reader should not have to guess which. The four files
// under `monster` and `enemy` are generated by scripts/srd/creatures.mjs from the SRD's own
// stat blocks — every number on them is printed in the source — and reviewed afterwards.
// `social.ts`' thirty NPCs have **no SRD source at all**, because the SRD has no innkeeper;
// they were written for this application and are the one part of the corpus checked against
// nothing but its own content rules. Which creatures were checked against what is a question
// somebody will ask, so it is answered here rather than inferred from a file listing.
//
// ⚠️ **Keys are links. ./retired.ts is where a key that stopped resolving is accounted for**,
// and `bestiary.test.ts` asserts that every key this corpus has ever published either still
// resolves or appears on that list with a reason.
//
...
// Re-exported through the barrel rather than reached for directly, so that "which keys does
// this corpus answer to" is one import. Read by `bestiary.test.ts` and by nothing in
// production — the list is a record for people, not a lookup for code.
export { RETIRED_ENTRIES, RETIRED_KEYS, type RetiredEntry, type RetirementReason } from './retired'
 ...
 * already holding, because two hundred and eighty-three rows built in one pass should not do
 * two hundred and eighty-three lookups to learn something the loop already knows. Two
 * callers, two right answers.
 *

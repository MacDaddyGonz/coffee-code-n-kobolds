...
describe('the stored species union carries what the narrow one will drop', () => {
  ...
  test('half-orc has a retirement label, and it is now the only thing that names it', () => {
    // ⚠️ **This test said something else on the branch it was written on, and the merge is
    // what changed the answer.** It read: *unreachable through `speciesLabel` today, because
    // `species('half-orc')` still resolves* — and asserted the label matched the entry's own
    // name, so that a sheet could not change its mind about the hyphen on the day the species
    // left. That was the widen half of widen → migrate → narrow applied to a *label*, written
    // on the schema branch while the content was still present on another.
    //
    // The species branch retired the content. Both branches were right about their own half;
    // what they could not see was each other, so the prediction and its fulfilment landed in
    // the same merge. This is the fulfilment.
    expect(RETIRED_SPECIES['half-orc']).toBe('Half-Orc')
    expect(species('half-orc')).toBeNull()

    // And the label is reachable now, which is the whole of what it was put there for: a
    // character built as a Half-Orc opens and says so, rather than showing a blank or a key.
    expect(speciesLabel('half-orc')).toBe('Half-Orc')
  })
})

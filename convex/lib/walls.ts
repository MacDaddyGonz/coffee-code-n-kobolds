...
/**
 * Every wall on one scene, again on another. For `scenes.duplicate`.
 *
 * ⚠️ **Unconditional, unlike the fog and the placements, and one sentence decides that:**
 * *a wall is a property of the map; a placement and a fog shape are where things are
 * tonight.* A DM who copies a dungeon level to lay out the next encounter wants the same
 * rooms with the same doorways, and re-tracing every corridor is the work the duplicate
 * button exists to avoid. So `includeContents` does not gate this — `scenes.duplicate` calls
 * it on both sides of that branch.
 *
 * `points` is copied by reference into a fresh row rather than rebuilt point by point. The
 * array is required on this table — see the schema — so there is no optional field here to
 * drop, which is the trap `copySceneFog` next door carries a warning about.
 */
export async function copySceneWalls(
  ctx: MutationCtx,
  fromSceneId: Id<'scenes'>,
  toSceneId: Id<'scenes'>,
): Promise<number> {
  const rows = await listWalls(ctx, fromSceneId)
  for (const row of rows) {
    await ctx.db.insert('walls', { sceneId: toSceneId, points: row.points })
  }
  return rows.length
}

/**
 * Multiply every wall on one scene by `k`. For `scenes.replaceImage`.
 *
 * The same uniform similarity transform `scaleSceneFog` applies, for the same reason: every
 * coordinate in this application is in the stored image's pixel space, so replacing the blob
 * moves everything, and one factor through both axes keeps a wall traced along a corridor
 * still along it. An aspect-ratio change is refused rather than scaled precisely so that one
 * factor is all there is.
 *
 * ⚠️ **A wall that did not scale is worse than a wall that vanished**, which is why this is
 * not optional cleanup. The map redraws at the new size and the barriers stay where they were
 * on the old one — so the party is stopped by nothing visible and walks through a door that is
 * plainly open. A missing wall at least looks like a missing wall.
 */
export async function scaleSceneWalls(
  ctx: MutationCtx,
  sceneId: Id<'scenes'>,
  k: number,
): Promise<number> {
  const rows = await listWalls(ctx, sceneId)
  for (const row of rows) {
    await ctx.db.patch('walls', row._id, {

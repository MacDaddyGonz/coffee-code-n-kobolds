...
// fallback is to assert against a literal 26 with a comment naming
...
describe('pipCapacity', () => {
  test('the capacity table', () => {
    expect(pipCapacity(26)).toBe(2)
    expect(pipCapacity(35)).toBe(3)
    expect(pipCapacity(46)).toBe(4)
    expect(pipCapacity(70)).toBe(6)
    expect(pipCapacity(140)).toBe(11)
    expect(pipCapacity(210)).toBe(TOKEN_MARKERS.length)
  })
  ...
  // `COIN_DETAIL_MIN_DIAMETER` across — and two pips need 22. Lower that threshold
  // to 20 and the smallest coins on the board become a bare `+4` counter with no
  // pip beside it, which is a worse answer than the name-only coin it replaced.
...
describe('MARKER_ROW_SCREEN_HEIGHT', () => {
  test('is the three vertical pieces of the row', () => {
    ...
    expect(MARKER_ROW_SCREEN_HEIGHT).toBe(15)
  })
})
...
describe('markerRow', () => {
  test('draws all of them when they fit', () => {
    ...
    expect(markerRow(['poisoned'], 26)).toEqual({ shown: ['poisoned'], overflow: 0 })
    // Exactly full is not a collapse: four in four slots is four pips.
    expect(markerRow(FOUR, 46)).toEqual({ shown: FOUR, overflow: 0 })
  })
  ...
  test('collapses into a counter that occupies the last slot', () => {
    ...
    expect(markerRow(FOUR, 35)).toEqual({ shown: ['blinded', 'charmed'], overflow: 2 })
    ...
    // that 26 holds two.
    expect(markerRow(FOUR, 26)).toEqual({ shown: ['blinded'], overflow: 3 })
  })
  ...
  test('gives the single slot to the counter rather than to one arbitrary pip', () => {
    ...
    expect(pipCapacity(12)).toBe(1)
    expect(markerRow(FOUR, 12)).toEqual({ shown: [], overflow: 4 })
    expect(markerRow(['poisoned'], 12)).toEqual({ shown: ['poisoned'], overflow: 0 })
  })
  ...
  test('drops a value it has never heard of', () => {
    ...
    expect(markerRow([...FOUR, 'exhausted', 'dazzled'], 35)).toEqual({
